...
export const Section: React.FC<Props> = ({ setCurrentSection }) => {
  ...
  const [addedSection, setAddedSection] = useState<SectionI | undefined>(
    undefined
  );
  ...
  const totalCourses = mockSections.reduce(
    (acc: number, section: any) => acc + section.courses.length,
    0
  );
  const allCoursesObj: any = {
    sectionTitle: "All courses",
    courses: [...mockSections],
    coursesSum: totalCourses,
  };
    ...
    <>
      <div className="w-[346px] fixed left-[84px] h-screen overflow-hidden">
        <div className="overflow-y-auto p-7 h-[90%] border-r border-[#EBEBEB] relative">
          {[allCoursesObj, ...mockSections, addedSection].map(
            (section, idx) => (
              <div
                key={idx}
                onClick={() => {
              ...
              >
                {sectionItem({
                  ...
                  numberOfCourses:
                    idx === 0 ? section?.coursesSum : section?.courses.length,
                  activeIndex: idx,
                })}
              </div>
            )
